...
async def seed_devices():
    ...
    if not all([device_models, statuses]):
        ...
        print(
            "👉 Пожалуйста, сначала запустите команду 'make init-data' для заполнения базовых данных."
        )
        return

    print(
        f'✅ Справочники успешно загружены. Начинаем создание {NUMBER_OF_DEVICES} активов...'
    )
    ...
    for i in range(NUMBER_OF_DEVICES):
        try:
            ...
            selected_department = (
                random.choice(departments + [None]) if departments else None
            )
            ...
            asset_data = {
                ...
                'warranty_end_date': faker.date_between(
            ...
            async with AsyncSessionFactory() as db:
                ...
                await device_service.create_device(
                    db, asset_to_create, user_id=ADMIN_USER_ID
                )
