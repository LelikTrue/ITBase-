...
class NetworkSettings(BaseMixin, Base):
    ...
    connection_type: Mapped[str | None] = mapped_column(
        Enum("access", "trunk", name="connection_type_enum"), default="access"
    )
    ...
    device: Mapped["Device"] = relationship(
        "Device", back_populates="network_settings", foreign_keys=[device_id]
    )
    ...
    switch: Mapped[Optional["Device"]] = relationship(
        "Device", foreign_keys=[switch_id], remote_side="Device.id"
    )
