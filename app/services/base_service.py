...
class BaseService(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def create(self, db: AsyncSession, obj_in: CreateSchemaType, user_id: int) -> ModelType:
        db_obj = self.model(**obj_in.model_dump())
        db.add(db_obj)
        await db.flush()
        await log_action(
            db=db, user_id=user_id, action_type='create',
            entity_type=self.model.__name__, entity_id=db_obj.id,
    ...
    async def update(
        ...
        old_data = {c.name: getattr(db_obj, c.name) for c in self.model.__table__.columns if hasattr(db_obj, c.name)}
        ...
        diff = {k: {'old': old_data.get(k), 'new': v} for k, v in update_data.items() if old_data.get(k) != v}
        if diff:
            await log_action(
    ...
    async def delete(self, db: AsyncSession, obj_id: int, user_id: int) -> ModelType | None:
        db_obj = await self.get_by_id(db, obj_id)
        if not db_obj:
            return None
        ...
        await log_action(
            ...
            entity_type=self.model.__name__, entity_id=obj_id,
    ...
    async def _check_duplicate(self, db: AsyncSession, field_name: str, value: Any, current_id: int | None = None):
        ...
        if result.scalar_one_or_none() is not None:
            raise DuplicateError(f"Запись в '{self.model.__name__}' с таким '{field_name}' уже существует.")

    async def _count_related(self, db: AsyncSession, related_field: Any, obj_id: int) -> int:
