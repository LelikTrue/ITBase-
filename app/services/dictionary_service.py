...
class DictionaryService:
    ...
    async def _create_item(self, db: AsyncSession, model: Type[Base], data: BaseModel) -> Base:
        ...
        # --- ИЗМЕНЕНИЕ 3: Добавляем обработку конкретной ошибки уникальности ---
            ...
            await db.rollback()
            logger.warning(f"Ошибка уникальности при создании записи в {model.__name__}: {e}")
            # Превращаем ошибку БД в наше исключение бизнес-логики
            raise DuplicateError("Запись с таким названием уже существует.")
        except SQLAlchemyError as e:
            await db.rollback()
            logger.error(f"Ошибка базы данных при создании записи в {model.__name__}: {e}", exc_info=True)
    ...
    async def create_device_model(self, db: AsyncSession, data: DeviceModelCreate) -> Base:
        from app.models import DeviceModel
        return await self._create_item(db, DeviceModel, data)

    async def create_device_status(self, db: AsyncSession, data: DeviceStatusCreate) -> Base:
        from app.models import DeviceStatus
        return await self._create_item(db, DeviceStatus, data)

    async def create_manufacturer(self, db: AsyncSession, data: ManufacturerCreate) -> Base:
