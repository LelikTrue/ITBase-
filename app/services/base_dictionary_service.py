...
class BaseDictionaryService:
    ...
    async def _check_duplicate(self, db: AsyncSession, name: str, item_id: Optional[int] = None):
    ...
    async def update(self, db: AsyncSession, item_id: int, data: DictionarySimpleUpdate, user_id: int) -> Optional[Base]:
    ...
    async def delete(self, db: AsyncSession, item_id: int, user_id: int) -> Optional[Base]:
        """Удаляет запись."""
        db_item = await db.get(self.model, item_id)
        if not db_item:
            return None
