    ...
    async def update_device_with_audit(
        ...
        try:
            ...
            await log_action(
                ...
                entity_type='Device', entity_id=db_device.id,
        ...
        try:
            for device in old_devices:
                ...
                if 'status' in new_related_models and device.status_id != new_related_models['status'].id:
                    diff['Статус'] = {'old': device.status.name if device.status else '', 'new': new_related_models['status'].name}
                if 'department' in new_related_models and device.department_id != new_related_models['department'].id:
                    diff['Отдел'] = {'old': device.department.name if device.department else '', 'new': new_related_models['department'].name}
                if 'location' in new_related_models and device.location_id != new_related_models['location'].id:
                    diff['Местоположение'] = {'old': device.location.name if device.location else '', 'new': new_related_models['location'].name}
