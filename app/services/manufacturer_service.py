...
class ManufacturerService(DuplicateCheckMixin, DependencyCheckMixin, BaseService[Manufacturer, DictionarySimpleCreate, DictionarySimpleUpdate]):
    ...
    async def create(self, db: AsyncSession, obj_in: DictionarySimpleCreate, user_id: int) -> Manufacturer:
    ...
    async def update(self, db: AsyncSession, obj_id: int, obj_in: DictionarySimpleUpdate, user_id: int) -> Manufacturer | None:
        # Уникальная логика: проверка дубликата по имени
        if obj_in.name:
