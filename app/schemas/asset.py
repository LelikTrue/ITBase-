...
class AssetBase(BaseModel):
    ...
    current_wear_percentage: int | None = Field(None, ge=0, le=100, description="Процент износа от 0 до 100")
    expected_lifespan_years: int | None = Field(None, ge=0, description="Ожидаемый срок службы в годах")
...
class AssetCreate(BaseFormModel, AssetBase): #!# Порядок наследования для корректной работы валидатора
    """Схема для создания нового актива. Наследует все поля из AssetBase."""
    pass # Все поля уже определены в AssetBase
...
class AssetUpdate(BaseFormModel):
    ...
    inventory_number: str | None = Field(None, max_length=255) # Только для чтения в форме
    serial_number: str | None = Field(None, max_length=255)
    mac_address: str | None = Field(None, max_length=255)
    ip_address: str | None = Field(None, max_length=255)
    notes: str | None = None
