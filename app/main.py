...
def load_admin_sections() -> List[Dict[str, str]]:
    ...
    # Конфигурационный файл должен находиться в app/config/
    config_path = Path(__file__).parent / 'config' / 'admin_sections.json'
    
    if not config_path.exists():
        logger.warning(f"Файл конфигурации разделов админ-панели не найден по пути: {config_path}")
        return []
    ...
    try:
        ...
        logger.error(f"Ошибка при загрузке файла конфигурации {config_path}: {e}")
        return []
