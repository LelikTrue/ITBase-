...
from fastapi import APIRouter, Depends, HTTPException, status, Request, Form # <-- ИЗМЕНЕНИЕ
from pydantic import EmailStr
from sqlalchemy.orm import selectinload
from sqlalchemy.ext.asyncio import AsyncSession
from typing import List, Optional # <-- ДОБАВЬ ЭТОТ ИМПОРТ, ЕСЛИ ЕГО НЕТ
...
from app.schemas.dictionary import (
    DictionarySimpleResponse, DictionarySimpleCreate, DictionarySimpleUpdate,
    DeviceModelCreate, DeviceModelUpdate, EmployeeCreate, EmployeeUpdate, DeviceModelResponse,
    EmployeeResponse, AssetTypeCreate, AssetTypeUpdate, AssetTypeResponse
)
# --- Новые импорты ---
...
from app.services.exceptions import DuplicateError, DeletionError, NotFoundError
...
async def update_employee(
    ...
    first_name: str = Form(...),
    patronymic: Optional[str] = Form(None),
    employee_id_str: Optional[str] = Form(None, alias="employee_id"), # Переименовано, чтобы избежать конфликта с employee_id из пути
    email: Optional[EmailStr] = Form(None),
    phone_number: Optional[str] = Form(None)
):
