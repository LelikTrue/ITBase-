...
async def read_assets(
    ...
    try:
        ...
        (
            paginated_devices_db,
            total_devices,
        ) = await device_service.get_devices_with_filters(
            db=db,
            page=page,
            page_size=page_size,
            sort_by=sort_by,
        ...
        paginated_devices = [
            AssetResponse.model_validate(d, from_attributes=True)
            for d in paginated_devices_db
        ]
        ...
        raise HTTPException(
            status_code=500, detail='Ошибка базы данных при загрузке активов.'
        )
...
async def dashboard(
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {"request": request, "title": "Дашборд - IT Asset Tracker"},
    )
...
async def export_assets_csv(
    ...
    try:
        ...
        devices = [
            AssetResponse.model_validate(d, from_attributes=True) for d in devices_db
        ]
        output = io.StringIO()
        output.write('\ufeff')
        ...
        for device in devices:
            writer.writerow(
                [
                    ...
                    f'{device.employee.last_name} {device.employee.first_name}'
                    if device.employee
                    else '',
...
async def add_asset_form(
    ...
    try:
        ...
        logger.error(
            f"Ошибка при загрузке данных для формы 'add_asset': {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail='Ошибка при загрузке данных для формы.'
        )
...
async def create_asset(
    ...
    logger.debug('Получены данные формы создания актива:')
    logger.debug(f'  Все ключи: {list(form_data.keys())}')
    logger.debug(f'  form_dict: {form_dict}')
    logger.debug(
        f'  Обязательные поля: name={form_dict.get("name")}, '
    ...
    try:
        ...
        await device_service.create_device(db=db, asset_data=asset_data, user_id=current_user.id)
...
async def edit_asset(
    ...
    try:
        ...
        logger.error(
            f'Ошибка при загрузке данных для формы редактирования: {e}', exc_info=True
        )
        raise HTTPException(
            status_code=500, detail='Ошибка при загрузке данных для формы.'
        )
...
@router.post(
    '/update/{device_id}', response_class=RedirectResponse, name='update_asset'
)
async def update_asset(
    ...
    try:
        ...
        updated_device = await device_service.update_device_with_audit(
            db=db, device_id=device_id, update_data=update_data, user_id=current_user.id
        )
...
async def bulk_delete_assets(
    ...
    try:
        deleted_count, errors = await device_service.bulk_delete_devices(
            db=db, device_ids=device_ids, user_id=current_user.id
        )
        if deleted_count > 0:
...
async def bulk_update_assets(
    ...
    try:
        ...
        updated_count = await device_service.bulk_update_devices(
            db=db, device_ids=device_ids, update_data=filtered_update_data, user_id=current_user.id
        )
