...
async def dashboard(
    ...
    try:
        ...
        context = {"request": request, "title": "Дашборд - IT Asset Tracker", **stats}
        return templates.TemplateResponse("dashboard.html", context)
    except SQLAlchemyError as e:
        logger.error(f"Ошибка при загрузке дашборда: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Ошибка базы данных при загрузке статистики."
        )
...
async def delete_asset(
    ...
    try:
        await device_service.delete_device_with_audit(
            db=db, device_id=device_id, user_id=1
        )
...
async def bulk_delete_assets(
    ...
    try:
        deleted_count, errors = await device_service.bulk_delete_devices(
            db=db, device_ids=device_ids, user_id=1
        )
        if deleted_count > 0:
