...
- `make logs-clear` — Очистить логи (пересоздание контейнеров).
...
- `make create-admin` — Создать администратора системы.
- `make db-shell` — Подключиться к PostgreSQL.
- `make rebuild-db` — Полная пересборка БД (с удалением данных).

### Качество кода

- `make lint` — Проверить стиль кода с помощью Ruff.
- `make lint-fix` — Автоматически исправить ошибки стиля (Ruff).
