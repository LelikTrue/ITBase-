...
8. **Теги** - `/admin/dictionaries/tags`

### ✅ Мониторинг и Аудит

- **Аналитический Дашборд** (`/dashboard`) - Финансовые метрики, графики статусов, таблица рисков.
- **Журнал Аудита** (`/audit-logs`) - История действий пользователей с фильтрацией и деталями изменений.

## 🚀 Как использовать

### 1. Доступ к админке

1. Откройте приложение: <http://localhost:8002>
...
1. **Финансы**: Карточки показывают общую стоимость и амортизацию.
2. **Графики**: Распределение по статусам и типам (Chart.js).
3. **Риски**: Таблица активов с истекающей гарантией или устаревших.

### 7. Журнал Аудита

1. **Просмотр**: Кто, когда и что изменил.
2. **Детали**: Нажмите "Показать" для просмотра diff (было/стало).
...
GET /api/dictionaries/tags                 # Список тегов
```
...
name=Новый тип&description=Описание
```
